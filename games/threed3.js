...
/* ---- RING RACE 3D ---- */
A.add({id:'rings',name:'RING RACE 3D',cat:'RETRO 3D',how:'FLY THROUGH EVERY RING. EACH ONE ADDS TIME.',make(){
 ...
 g.draw=()=>{sky('#1a2a6a','#ff9838');R(0,130,W,110,'#1e5a35');A.cam.x=px*.9;A.cam.y=py*.9+.6;A.cam.z=z-4;A.cam.ry=0;A.cam.rx=-.08;
  ...
  rings.forEach(r=>{if(r.z>z-1&&r.z<z+60)ring3(r.x,r.y,r.z,1.3,r.ok?K.gr:r.z-z<12?K.y:K.o);});A.flush();
...
/* ---- CANYON RUN 3D ---- */
A.add({id:'canyon',name:'CANYON RUN 3D',cat:'RETRO 3D',how:'FLY THE CANYON. WALLS CLOSE IN. DON\'T TOUCH.',make(){
 ...
 g.draw=()=>{sky('#ff9838','#ffcf3f');A.cam.x=px;A.cam.y=1.5;A.cam.z=z-3;A.cam.ry=0;A.cam.rx=-.05;
  ...
  A.flush();const s=A.p3(px,1,z+1.5);A.poly([[s[0],s[1]-8],[s[0]-16,s[1]+6],[s[0],s[1]+2],[s[0]+16,s[1]+6]],K.c,1);T(g.score,160,6,K.w,3,'c');};
 return g;}});

/* ---- BRICK BREAKER 3D ---- */
A.add({id:'bricks3d',name:'BRICK BREAKER 3D',cat:'RETRO 3D',how:'MOVE THE PADDLE. A LAUNCHES. CLEAR THE WALL.',make(){
...
/* ---- HOVER TANK 3D ---- */
A.add({id:'hover',name:'HOVER TANK 3D',cat:'RETRO 3D',how:'TURN + DRIVE. A FIRES. DESTROY 10 DRONES.',make(){
 ...
 g.draw=()=>{sky('#05030f','#2b2257');R(0,130,W,110,'#1a1440');A.cam.x=p.x-Math.sin(p.a)*5;A.cam.z=p.z-Math.cos(p.a)*5;A.cam.y=2.5;A.cam.ry=-p.a;A.cam.rx=-.3;
  ...
  if(inv%6<4){B3(p.x,.2,p.z,1.6,.6,2,K.c);const c=Math.sin(p.a),s=Math.cos(p.a);B3(p.x+c*.8,.9,p.z+s*.8,.3,.3,1.4,'#178a7d');}A.flush();
  T('SCORE '+g.score,6,4,K.y,2);T('ARMOUR '+'|'.repeat(hp),W-6,4,hp<2?K.r:K.w,2,'r');};
 return g;}});

/* ---- ROLLER 3D ---- */
A.add({id:'roller',name:'ROLLER 3D',cat:'RETRO 3D',how:'ROLL THE BALL. A JUMPS. STAY ON THE PATH.',make(){
 const g={over:null,score:0};let p={x:0,y:0,z:0,vx:0,vy:0,vz:.12},tiles=[],spin=0;for(let i=0;i<40;i++)tiles.push({x:Math.round(Math.sin(i*.4)*2),z:i*2,w:i%7===6?0:3});
 ...
 g.update=()=>{const k=A.in(0);p.vx=(p.vx+ax(k)*.02)*.9;p.x+=p.vx;p.z+=p.vz;spin+=p.vz;g.score=p.z|0;const ground=on()&&p.y<=0.01;if(ground){p.y=0;p.vy=0;if(A.hit(0).a){p.vy=.22;S('jump');}}else{p.vy-=.012;}p.y+=p.vy;if(p.y<-6){g.over='FELL OFF';S('boom');}
  for(const t of tiles)if(t.z<p.z-4){t.z+=80;t.x=Math.round(Math.sin(t.z*.2)*2);t.w=Math.random()<.15?0:3;}};
 g.draw=()=>{sky('#1a0a44','#ff3f8e');R(0,130,W,110,'#12082a');A.cam.x=p.x;A.cam.y=p.y*.5+3;A.cam.z=p.z-5;A.cam.ry=0;A.cam.rx=-.35;
  ...
  const r=.45,y=p.y+r;for(let i=0;i<4;i++){const a=spin+i*1.57;F([[p.x-r,y+Math.sin(a)*r*.6,p.z+Math.cos(a)*r*.6],[p.x+r,y+Math.sin(a)*r*.6,p.z+Math.cos(a)*r*.6],[p.x+r,y+Math.sin(a+1.57)*r*.6,p.z+Math.cos(a+1.57)*r*.6],[p.x-r,y+Math.sin(a+1.57)*r*.6,p.z+Math.cos(a+1.57)*r*.6]],i%2?K.y:K.o);}A.flush();T(g.score,160,6,K.w,3,'c');};
 return g;}});

/* ---- WAVE RIDER 3D ---- */
A.add({id:'waverider',name:'WAVE RIDER 3D',cat:'RETRO 3D',how:'STEER BETWEEN THE BUOYS. RED LEFT, GREEN RIGHT. 60 SEC.',make(){
 ...
 g.draw=()=>{sky('#4dabff','#9fd8ff');A.cam.x=px;A.cam.y=1.4;A.cam.z=z-3.5;A.cam.ry=0;A.cam.rx=-.12;
  ...
  B3(px,.1,z,.7,.4,1.6,K.y);B3(px,.5,z-.2,.4,.5,.6,K.c);A.flush();T('SCORE '+g.score,6,4,K.y,2);T(Math.ceil(time/60),W-6,4,K.w,2,'r');};
 return g;}});
})();
